...
//        }).collect(Collectors.toList());
    ...
    public AjaxResult enableUser(@PathVariable Long[] userIds)
    {
        AjaxResult ajax = AjaxResult.success();
        userService.enableUserByIds(userIds);
        return ajax;
    }

    @PutMapping("/disable/{userIds}")
    public AjaxResult disableUser(@PathVariable Long[] userIds)
    {
        AjaxResult ajax = AjaxResult.success();
        userService.disableUserByIds(userIds);
        return ajax;
    }
...
//        userService.checkUserDataScope(user.getUserId());
     ...
     * 获取代理上级用户列表
     */
    @GetMapping("/parentUserList")
    public TableDataInfo parentUserList(SysUser user)
    {
        startPage();
        List<ParentUserListRespVO> list = userService.selectParentUserList(user);
