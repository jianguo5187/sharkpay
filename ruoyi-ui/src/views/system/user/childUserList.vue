...
<!--          </el-col>-->
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange" @sort-change='sortTableFun'>
          ...
          <el-table-column label="ID" align="center" key="userId" prop="userId" width="80" sortable="custom"/>
...
export default {
  ...
  components: { Treeselect ,BetRealTime},
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleEnableUser(row) {
      ...
      this.$modal.confirm('是否确认启用用户编号为"' + userIds + '"的数据项？').then(function() {
    ...
    handleDisableUser(row) {
      const userIds = row.userId || this.ids;
      this.$modal.confirm('是否确认禁用用户编号为"' + userIds + '"的数据项？').then(function() {
        return disableUser(userIds);
      }).then(() => {
        this.getList();
